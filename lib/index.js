...
exports.runCommand = function (cmd, pidsObj, callback) {
    ...
    if (Fs.existsSync(internals.Smelt.settings.dirPath)) {
        ...
        var origDir = process.cwd();
        process.chdir(internals.Smelt.settings.dirPath);
        var cmdResult = Child.spawn(command.cmd, command.args, { cwd: internals.Smelt.settings.dirPath, stdin: 'inherit', detached: true });
        //console.log(cmdResult.pid);
        //var cmdResult = Child.exec(cmd, { cwd: internals.Smelt.settings.dirPath, stdin: 'inherit' });
        process.chdir(origDir);
        result.command = cmd;
        result.pid = cmdResult.pid;
...
exports.runShellCommand = function (command, pidsObj, callback) {

    internals.Smelt.runCommand('/bin/sh -c ' + command, pidsObj, function (result) {
